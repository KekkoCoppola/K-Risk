...
**Attenzione per la tesi**: gli autori descrivono un dataset trasversale di **pazienti con diabete** del reparto, ma i dati dicono altro. Nel training `DM` = 1 riguarda solo il 6% dei soggetti, e fra chi ha `DM` = 0 soltanto 75 su 4.087 (l'1,8%) superano una soglia diagnostica del diabete (glicemia a digiuno ≥ 7,0 mmol/L, HbA1c ≥ 6,5% o glicemia a 2 ore ≥ 11,1 mmol/L; ADA 2024, Tabella 2.1), con mediane di 5,45 mmol/L e 5,5% [T28]. Il dizionario dei dati contiene variabili tipiche della valutazione del rischio di diabete: glicemia classificata in normale, prediabete e diabete, diabete di nuova diagnosi, popolazione ad alto rischio [T28b]. La lettura più plausibile è una popolazione **in gran parte senza diabete, valutata da un centro ospedaliero per il diabete**, con un reclutamento non documentato: non è uno screening della popolazione generale (un solo reparto), ma nemmeno una coorte di soli diabetici. Il tipo di campione urinario e le unità di `UCRE`, `UmALB` e `UMAUCR` **non sono documentati** (nel dizionario dei dati degli autori l'unità è "None"), e nel training `UMAUCR` vale 176,8 × `UmALB`/`UCRE` su ogni riga, un fattore che non corrisponde a nessuna conversione standard (sezione 6bis) [T29]. Ci sono piccole incongruenze interne (dati trasversali o di follow-up, durata della raccolta) [T30]. K-Risk va quindi presentato come modello sviluppato su una coorte ospedaliera cinese, da validare in popolazioni di screening.
...
- *Formulazione onesta*: "I dati vengono da un reparto ospedaliero di diabetologia. Gli autori parlano di pazienti con diabete, ma nel training `DM` = 1 riguarda solo il 6% e fra gli altri solo l'1,8% supera una soglia diagnostica del diabete: sono in gran parte persone senza diabete valutate da un centro per il diabete, con un reclutamento non documentato. Non è uno screening della popolazione generale."
- *Argomento di valore*: la discrepanza è stata trovata e misurata dal progetto, non presa per buona dalla descrizione degli autori. E la popolazione reale, in gran parte senza diabete, è proprio quella a cui serve uno strumento per decidere chi mandare all'esame delle urine: le linee guida raccomandano lo screening annuale dell'albuminuria solo nelle persone con diabete (sezione 1). L'analisi per sottogruppi mostra dove il modello serve (fra i non diabetici) e dove no (fra i diabetici, che le linee guida mandano comunque all'esame). Il modello resta sviluppato su una coorte ospedaliera cinese, da validare in popolazioni di screening.
- *Prove*: sezioni 1 e 5; `DM`, `FPG`, `HbA1c` e `PG2h` nel training; dizionario dei dati del dataset (p. 3 e 6).

---

## 9. Limiti da dichiarare

- dataset **ospedaliero** (un reparto di diabetologia, Shanghai, 2012), descritto dagli autori come dati di pazienti con diabete ma con `DM` = 1 nel 6% del training e una soglia diagnostica del diabete superata solo dall'1,8% degli altri; reclutamento non documentato, non uno screening della popolazione generale: la validità esterna va dimostrata;
...
- ADA Professional Practice Committee. 2. Diagnosis and Classification of Diabetes: Standards of Care in Diabetes—2024. *Diabetes Care* 2024;47(Suppl 1):S20-S42. doi:10.2337/dc24-S002. PMID 38078589 (TC: Tabella 2.1)
...
| T28 | "pazienti con diabete" contro DM = 1 nel 6%; 75 su 4.087 (1,8%) sopra una soglia diagnostica; mediane 5,45 mmol/L e 5,5% | doi:10.1038/s41597-026-06923-y, Methods ("cross-sectional dataset of diabetes patients"), TC; quota: come T25; soglie: ADA 2024, sezione 2, Tabella 2.1, doi:10.2337/dc24-S002 (PMC10725812, TC), le stesse del dizionario del dataset (`FPGover7`, `P2PGover11`, p. 6); conteggi e mediane su `data/processed/train.csv` (S18); l'HbA1c è misurata con un dispositivo portatile e manca in 369 dei 4.087 |
| T28b | variabili di valutazione del rischio di diabete | dizionario dei dati del dataset (`data/raw/Supplementary Information.pdf`): `PGclass3` normale / prediabete / diabete e `DMnew3class` diabete di nuova diagnosi (p. 3), `DMhighrisk` popolazione ad alto rischio (p. 6) |
